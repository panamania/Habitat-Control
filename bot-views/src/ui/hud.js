...
export class Hud {
  ...
  _wire() {
    ...
    on('#btn-ask', 'click', () => this._sendAsk())
  ...
  /**
   * Send whatever is in the ask box for the selected thread, then clear it. Disabled while a
   * request is in flight so Enter-mashing can't fire the same prompt twice — the button and
   * input both come back whether it succeeded or not, since the toast already says which.
   */
  _sendAsk() {
    const input = this.$('#ask-input')
    const prompt = input.value.trim()
    if (!prompt || !this.selected) return
    input.disabled = true
    this.$('#btn-ask').disabled = true
    Promise.resolve(this.actions.askAgent?.(prompt)).finally(() => {
...
<aside class="side panel">
  <header class="brandbar">
    <div class="brand"><i class="dot"></i>Habitat Control</div>
