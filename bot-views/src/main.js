...
const actions = {
  ...
  /**
   * Send a prompt to the selected thread's agent. A high-stakes zone (see zones.json)
   * answers 409 with requiresConfirmation instead of running it — that's the server asking
   * to be told again on purpose, not an ordinary failure, so it gets its own confirm step
   * rather than just landing in the error toast.
   *
   * askAgent() itself only settles once the server has queued the prompt (a 202, not the
   * agent's actual reply — see api.js) — this resolves as soon as that happens, so the ask
   * box's disabled state clears right away rather than sitting locked for however long the
   * agent takes. What the agent said arrives later, over subscribeInvocation, as its own
   * toast; a slow agent means a slow second toast, not a stuck input.
   */
    ...
    try {
      ...
      if (err.requiresConfirmation) {
        ...
        try {
          ...
          hud.toast(err2.message || 'Could not reach that agent', 'err')
        }
        return
      }
      hud.toast(err.message || 'Could not reach that agent', 'err')
    }
  },
